...
1. **團隊是否使用 AI Coding Agent？** → 否 → 暫時跳過 ZeroSpec，待日後重新評估。
2. **能否寫出至少 3 條「寫錯會導致 PR 退件」的硬規則？** → 否 → 先穩定架構；空殼的 AGENTS.md 沒有價值。
3. **這是已有大量現有 API 的專案嗎？** → 是 → **Brownfield 路線**：執行 INIT-SCAN + INIT-BUILD，再接續 SA Prompt，之後漸進補齊優先 SPEC（見 Step 3.5：依專案類型選擇下一步）。
4. **這是新專案或小型專案？** → 是 → **Greenfield 路線**：執行 INIT-SCAN + INIT-BUILD，然後讓第一個實際 API 自然觸發第一張 SPEC。
5. **還是不確定？** → 參考 README.zh-TW.md 的 30 秒起步步驟。完成一項真實任務後，若 `AGENTS.md` 感覺有用，就是正確方向。
---
...
當某一類文件數量達到下方定義的檔案數門檻時，在該類別目錄內新增**薄索引**以支援 intent-driven 導航。

#### 觸發條件

當 `docs/spec/` 目錄內符合 `SPEC-*.md` 的檔案 **≥ 8 份**時，建立 `docs/spec/README.md`。

此門檻為確定性檢查（列出目錄並計算檔案數），非主觀判斷。AI 代理可透過列目錄直接評估。

#### 適用範圍

目前僅定義 **SPEC**。其他文件類型若未來沒有明確新增規則，不適用子索引規則。

#### 子索引包含內容

使用 [`templates/SPEC-INDEX-TEMPLATE.md`](templates/SPEC-INDEX-TEMPLATE.md) 作為骨架。必要段落：

1. **Purpose** — 範圍宣告 + SoT 提醒
2. **Document Index** — 所有 SPEC 的表格（名稱、主題、範圍、讀者、狀態）
3. **How to Choose** — 情境對應 SPEC 映射（intent-driven，非 filename-driven）
4. **Maintenance Rules** — 將程式碼變更類型映射到 SPEC 更新義務
5. **Status Guide** — 定義索引中使用的狀態標籤

**禁止內容**：端點表、DTO 結構、業務規則或任何屬於個別 SPEC 的契約細節。子索引是導航層，不是規格層。

**語言規則**：人類可讀的 README 內容（標題、說明文字、表格欄位、情境描述、維護規則說明、狀態含義）應依專案偵測到的文件語言或明確的 `Respond in {locale}` 覆寫進行本地化。檔案路徑、程式識別字、SPEC 檔名、指令與連結維持原文。

#### 與 `docs/README.md` 的關係

- `docs/README.md` **保留** SPEC 扁平清單（名稱 + 路徑 + 狀態）——確保向後相容並提供一覽式總覽。
- `docs/README.md` **額外連結**至 `docs/spec/README.md` 以提供情境式查找。
- 兩層各司其職：頂層 = 快速位置定位；子索引 = intent-driven 導航 + 維護規則映射。這不是 SoT 重複。

#### 維護職責

| 動作                                       | 負責的 Prompt                                                    |
| ------------------------------------------ | ---------------------------------------------------------------- |
| **建立**子索引（首次）                     | `UPDATE.md` — 在定期稽核中，當門檻達標時                         |
| **新增或更新**一行（SPEC 異動時）          | `SPEC.md` — 在 Post-Output Verification 中（僅在子索引已存在時） |
| **更新** How to Choose / Maintenance Rules | `UPDATE.md` — 在定期稽核中（跨 SPEC 全局視角）                   |

`SPEC.md` 永遠不建立子索引結構，只新增或更新已存在索引中的列。

#### 不可逆性

子索引一旦建立，即使 SPEC 數量日後降至門檻以下也不移除。簡短的索引不會造成損害；移除則可能破壞既有的導航連結。
...
完整清單請見 [anti-patterns.zh-TW.md](anti-patterns.zh-TW.md)。以下為最常見的項目：
...
任何可選配套（例如 IDE Prompt File 範本、快捷入口樣板）都應該能證明自身存在價值：

- 新增配套若在 **30 天內未被任何 PR 引用或 Issue 提及**，在下次 quarterly review 時降級為 `examples/` 或直接移除。
- 量測方式：`git log --all --oneline --grep="templates/prompts"` 查看引用次數。
- 原則：維護成本會累積，先移除再新增。
