...
| 25  | **改了程式碼卻不評估 SPEC** | AI 完成程式異動（新端點、schema 變更、Permission 變更、行為性 bugfix）後，沒有評估是否需要更新 SPEC，文件與程式碼悄悄脫鉤 | 在 `AGENTS.md` 加入 `## Post-Edit Self-Check` 段落，並在 `## Quick Constraints` 首條強制要求 SPEC 評估。複雜多模組任務改用 IMPL Prompt Pack。見 [`prompts/IMPL.md`](prompts/IMPL.md)。 |
