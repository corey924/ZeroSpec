...
| **關鍵約束（Quick Constraints）** | 5–8 條最致命硬規則置頂摘要 | C — 人提供  | 從規範中萃取，長對話稀釋時仍在前 500 tokens 存活 |
...
| 2 | 關鍵約束（Quick Constraints） | 違反時直接導致 PR 退件的硬規則，集中 5–8 條 |
...
**Quick Constraints 設計要點**：從「程式碼產生規範」中提取 5–8 條最致命的硬規則（違反時會導致 PR 退件或系統錯誤），集中放在「專案定位」之後。即使後段被對話歷史稀釋，核心約束仍在前 500 tokens 存活。
