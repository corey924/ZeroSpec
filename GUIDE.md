...
**強調語法使用注意**：Quick Constraints 本身已由專案定位段落置頂，語意上就是「最重要的 5–8 條規則」。**不建議**在每條前都加 `IMPORTANT:` 或 `YOU MUST`；一旦每條都有強調語，等於沒有強調。僅在某條規則已經經過 Context Hygiene、每行自檢、解歧義等手段後仍被 AI 反覆違反，才在該單條規則前加強調語（詳見 [DAILY-USAGE §5.6](DAILY-USAGE.md#56-ai-反覆違反同一條-agentsmd-規則) 診斷流程）。
